...
- Dataset Name: This is the S3 prefix where the dataset and pre-processing automation code resides. For this e.g., it's `provisional-death-count-covid19-county`
- Product Name: This is the product name on ADX. For this e.g., it's `Provisional COVID-19 Death Counts in United States by County | CDC NCHS`
...
`./init.sh --s3-bucket "rearc-data-provider" --dataset-name "provisional-death-count-covid19-county" --product-name "Provisional COVID-19 Death Counts in United States by County | CDC NCHS" --product-id "blank" --region "us-east-1"`
